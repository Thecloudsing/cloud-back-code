...
@Service("GetForgetCode")
public class GetForgetCode implements TypeProcessing {
    @Override
    ...
    private void getCode(String account, String socketValue) throws IOException {
        ...
        if (uid != null) {
            String forgetCode = codeData.addForgetCode(account);
            sendMail.registerMail(account, forgetCode);
            new ValidationReturns(socketValue).registerStatus(TCP_Type_Client.FORGET_GET_CODE_SUCCESS, null);
        } else
            new ValidationReturns(socketValue).registerStatus(TCP_Type_Client.FORGET_GET_CODE_FAILURE, null);
    }
}
