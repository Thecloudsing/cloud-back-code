...
public class AuthorizationController {
    ...
    public String activateRedirect(HttpSession session,
        ...
        UriComponentsBuilder uriBuilder = UriComponentsBuilder
                .fromUriString(customSecurityProperties.getDeviceActivatePageUri())
                .queryParam("userCode", userCode)
                .queryParam(SecurityConstants.NONCE_HEADER_NAME, session.getId());
    ...
    public Result<String> consentRedirect(HttpSession session,
                                          ...
                                          @RequestParam(OAuth2ParameterNames.STATE) String state,
                                          @RequestParam(OAuth2ParameterNames.CLIENT_ID) String clientId,
                                          @RequestHeader(name = SecurityConstants.NONCE_HEADER_NAME, required = false) String nonceId,
                                          @RequestParam(name = OAuth2ParameterNames.USER_CODE, required = false) String userCode) {
        ...
        UriComponentsBuilder uriBuilder = UriComponentsBuilder
                ...
                .queryParam(SecurityConstants.NONCE_HEADER_NAME, ObjectUtils.isEmpty(nonceId) ? session.getId() : nonceId);

        String uriString = uriBuilder.build(Boolean.TRUE).toUriString();
        if (ObjectUtils.isEmpty(userCode) || !UrlUtils.isAbsoluteUrl(customSecurityProperties.getDeviceActivatePageUri())) {
